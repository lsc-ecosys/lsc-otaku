...
func (a *App) CheckUpdateByVersion(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("stable: " + (*closestArtifactVersion)[RELEASESTABLE].Version)
	versionCompared, err = utils.CompareSemVers(version, (*closestArtifactVersion)[RELEASESTABLE].Version)
	fmt.Printf("versionCompared: %d", versionCompared)
	if err != nil {
		responses.ERROR(w, http.StatusInternalServerError, err)
		return
	}
	...
	rollout, err = models.FindAvailableRollout(a.DB, (*closestArtifactVersion)[RELEASESTABLE].Version)
	resp["data"] = rollout
	responses.JSON(w, http.StatusOK, resp)
	return
}
...
func GetClosestArtifactVersion(version string, a *App) (*map[string]*ReleaseArtifact, error) {
	...
	for i := 0; i < lengthOfAfs; i++ {
		...
		if relTag[release] == nil {
			fmt.Println("relTag[release] is nil")
			relTag[release] = new(ReleaseTag)
		}
		if relTag[release] != nil && relTag[release].Bound == true {
			continue
		}
	...
	for rel := range relTag {
		...
		if idx != -1 {
			if releases[rel] == nil {
				releases[rel] = new(ReleaseArtifact)
			}
			...
			fmt.Println("releases[rel].Version: " + releases[rel].Version)
